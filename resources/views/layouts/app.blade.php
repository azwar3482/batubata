...
<body class="antialiased bg-[#f8fafc] dark:bg-slate-900 text-slate-800 dark:text-slate-200 relative overflow-hidden transition-colors duration-300">
    ...
    <div x-data="{ 
        ...
        <aside :class="sidebarOpen ? 'translate-x-0 w-[280px] sm:w-64' : '-translate-x-full lg:translate-x-0 mini-sidebar'"
            class="fixed inset-y-0 left-0 z-50 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 shadow-xl lg:shadow-none transform transition-[width,transform] duration-300 ease-in-out lg:static flex flex-col h-full">
            ...
            <div class="flex-1 pb-3 sm:pb-4" :class="sidebarOpen ? 'overflow-y-auto overflow-x-hidden' : 'overflow-visible'">
                ...
                <nav class="px-2 sm:px-3 space-y-0.5 sm:space-y-1 sidebar-nav h-full">
                    ...
                    <a href="{{ route('dashboard') }}"
                        class="group flex items-center menu-link px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-medium rounded-lg sm:rounded-xl transition-[opacity,margin] duration-300  {{ request()->routeIs('dashboard*') ? 'bg-gradient-to-r from-blue-50 to-indigo-50/50 text-blue-700 font-semibold border-l-4 border-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-50/80 hover:text-slate-900' }}">
        ...
        <!-- Overlay Mobile -->
        <div x-show="sidebarOpen" @click="sidebarToggle()"
