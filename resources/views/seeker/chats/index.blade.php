    ...
    <div class="py-6 h-[calc(100vh-64px)] overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col">
            ...
            <div class="mb-4 {{ $activeConversation ? 'hidden md:block' : '' }}">
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400">Balas obrolan langsung dari perusahaan/industri penawar kerja.</p>
            </div>

            <!-- Info Card -->
            <div class="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-100 dark:border-blue-800/50 rounded-xl {{ $activeConversation ? 'hidden md:block' : '' }}">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-blue-900 dark:text-blue-200 mb-1">Tentang Pesan Langsung</h4>
                        <p class="text-sm text-blue-700 dark:text-blue-300 leading-relaxed">Fitur ini memungkinkan Anda <strong>berkomunikasi langsung</strong> dengan pihak industri/perusahaan yang tertarik dengan profil Anda. Hanya perusahaan yang dapat memulai percakapan terlebih dahulu. Balas pesan dengan sopan untuk meningkatkan peluang Anda diterima.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 min-h-0 bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-gray-100 dark:border-slate-800 flex overflow-hidden mb-6"
                ...
                <div class="w-full md:w-80 lg:w-96 border-r border-gray-100 dark:border-slate-800 flex flex-col {{ $activeConversation ? 'hidden md:flex' : 'flex' }}">
                    ...
                    <div class="p-4 border-b border-gray-100 dark:border-slate-800">
                        <div class="relative">
                            <input type="text" x-model="searchQuery" placeholder="Cari perusahaan..." 
                    ...
                    <div class="flex-1 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-800/50">
                        @forelse($conversations as $conv)
                            @php
                                ...
                                $companyName = $conv->industry->company->name ?? $conv->industry->name;
                            @endphp
                            <a href="{{ route('seeker.chats.index', $conv->id) }}" 
                               ...
                               x-show="matchesSearch('{{ addslashes($companyName) }}')">
                                
                                <!-- Company Initials Avatar -->
                                <div class="w-11 h-11 rounded-full bg-gradient-to-tr from-gray-700 to-gray-900 text-white font-bold flex items-center justify-center text-sm shrink-0 shadow-sm">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $companyName }}</h4>
                        ...
                        @empty
                            <div class="p-8 text-center text-gray-500 dark:text-slate-400">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Hanya pihak industri/perusahaan yang dapat memulai obrolan dengan Anda.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="flex-1 flex flex-col bg-slate-50/50 dark:bg-slate-900/40 {{ $activeConversation ? 'flex' : 'hidden md:flex' }}">
                    @if($activeConversation)
                        @php
                            $activeCompanyName = $activeConversation->industry->company->name ?? $activeConversation->industry->name;
                        @endphp
                        <!-- Pane Header -->
                        <div class="px-6 py-4 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between shrink-0 shadow-sm">
                            <div class="flex items-center gap-3">
                                <!-- Mobile Back Button -->
                                <a href="{{ route('seeker.chats.index') }}" class="p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-white rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition md:hidden mr-1">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                                    </svg>
                                </a>

                                <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-gray-700 to-gray-900 text-white font-bold flex items-center justify-center text-sm">
                                    {{ substr($activeCompanyName, 0, 2) }}
                                </div>
                                <div>
                                    <h3 class="text-sm font-bold text-gray-900 dark:text-white leading-tight">{{ $activeCompanyName }}</h3>
                                    <p class="text-xs text-gray-400 leading-none mt-1">Recruiter: {{ $activeConversation->industry->name }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex-1 overflow-y-auto p-6 space-y-4" id="messages-container">
                            ...
                            @foreach($messages as $msg)
                                ...
                                <div class="flex {{ $isMe ? 'justify-end' : 'justify-start' }} items-end gap-2">
                                    @if(!$isMe)
                                        <div class="w-6 h-6 rounded-full bg-gradient-to-tr from-gray-700 to-gray-900 text-white font-bold flex items-center justify-center text-[9px] shrink-0">
                                            {{ substr($activeCompanyName, 0, 2) }}
                                        </div>
                                    @endif
                        ...
                        <div class="px-6 py-4 bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 shrink-0">
                            <form action="{{ route('seeker.chats.send', $activeConversation->id) }}" method="POST"
                                  @submit.prevent="submitMessage($el)">
                                @csrf
                                <div class="flex gap-2 items-end">
                                    <div class="flex-1 relative">
                                        <textarea name="message" x-model="newMessage" rows="1" max-length="5000" required
                                                  placeholder="Ketik balasan..." @keydown.enter.prevent="if(newMessage.trim()){ $el.closest('form').dispatchEvent(new Event('submit')) }"
                    ...
                    @else
                        ...
                        <div class="flex-1 flex flex-col items-center justify-center p-8 text-gray-400 dark:text-slate-500">
                            ...
                            <p class="text-sm text-center mt-1">Pilih salah satu perusahaan di sebelah kiri untuk melihat pesan dan membalas obrolan.</p>
                        </div>
                    @endif
                </div>
