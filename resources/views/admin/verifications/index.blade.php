<x-app-layout>
    <div class="py-12 bg-gray-50 dark:bg-slate-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div x-data="{ activeTab: '{{ request()->has('institutions_page') ? 'education' : 'industry' }}' }" class="bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-gray-200 dark:border-slate-700 overflow-hidden">
                <!-- Tabs Navigation -->
                <div class="border-b border-gray-200 dark:border-slate-700 px-6 pt-4 flex space-x-6">
                    <button @click="activeTab = 'industry'" 
                            :class="{ 'border-indigo-600 text-indigo-600 dark:text-indigo-400': activeTab === 'industry', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-slate-400 dark:hover:text-slate-300': activeTab !== 'industry' }"
                            class="whitespace-nowrap pb-4 px-1 border-b-2 font-bold text-sm transition-colors duration-200">
                        Verifikasi Industri
                    </button>
                    <button @click="activeTab = 'education'" 
                            :class="{ 'border-blue-600 text-blue-600 dark:text-blue-400': activeTab === 'education', 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-slate-400 dark:hover:text-slate-300': activeTab !== 'education' }"
                            class="whitespace-nowrap pb-4 px-1 border-b-2 font-bold text-sm transition-colors duration-200">
                        Verifikasi Edukasi
                    </button>
                </div>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 dark:text-slate-400 font-medium">Tidak ada data verifikasi industri.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            <thead class="bg-gray-50 dark:bg-slate-900/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider w-16">No</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Institusi</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Tanggal Pengajuan</th>
                                    <th class="px-6 py-4 text-right text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Aksi</th>
                                </tr>
                            ...
                            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-slate-700">
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            <div class="h-10 w-10 rounded-xl bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center font-bold text-lg flex-shrink-0">
                                                {{ substr($institution->name, 0, 1) }}
                                            </div>
                                            <div>
                                                <div class="font-semibold text-gray-900 dark:text-white">{{ $institution->name }}</div>
                                                <div class="text-xs text-gray-500 dark:text-slate-400">{{ ucfirst($institution->type) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            @if($institution->isVerified())
                                            <form action="{{ route('admin.institutions.verifications.re-review', $institution->id) }}" method="POST" onsubmit="return confirm('Yakin ingin melakukan tinjau ulang? Status verifikasi akan direset ke pending.')">
                                                @csrf
                                                <button type="submit" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-amber-600 bg-amber-50 border border-amber-200 hover:bg-amber-100 rounded-lg transition-colors">
                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                                                    Tinjau Ulang
                                            ...
                                            @endif
                                            <a href="{{ route('admin.institutions.verifications.show', $institution->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 border border-blue-200 hover:bg-blue-100 rounded-lg transition-colors">
                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                Tinjau
                                            </a>
                                        </div>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 dark:text-slate-400 font-medium">Tidak ada data verifikasi edukasi.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
