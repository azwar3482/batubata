    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6 anim-1">
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">Detail penilaian hasil Tes Potensi Akademik dari {{ $result->user->name }}.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="divide-y divide-gray-100 dark:divide-slate-800">
                        ...
                        <div class="p-5 hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors {{ $answer->is_correct ? 'border-l-4 border-l-emerald-500' : ($answer->selected_answer ? 'border-l-4 border-l-rose-500' : 'border-l-4 border-l-slate-300 dark:border-l-slate-650') }}">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1 space-y-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs px-2.5 py-0.5 rounded-full font-medium {{ $answer->question->category_label == 'Verbal' ? 'bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300 border border-blue-100 dark:border-blue-900/30' : ($answer->question->category_label == 'Numerik' ? 'bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300 border border-purple-100 dark:border-purple-900/30' : ($answer->question->category_label == 'Logika' ? 'bg-orange-50 text-orange-700 dark:bg-orange-900/20 dark:text-orange-300 border border-orange-100 dark:border-orange-900/30' : 'bg-teal-50 text-teal-700 dark:bg-teal-900/20 dark:text-teal-300 border border-teal-100 dark:border-teal-900/30')) }}">
                                            {{ $answer->question->category_label }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-gray-750 dark:text-slate-300 leading-relaxed">{!! Str::limit(strip_tags($answer->question->question_text), 180) !!}</p>
                                </div>
                                <div class="text-right shrink-0">
                                    @if($answer->is_correct)
                                    <div class="inline-flex items-center gap-1 px-3 py-1 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 rounded-full text-xs font-semibold border border-emerald-100 dark:border-emerald-900/30">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                        Benar
                                    </div>
                                    @elseif($answer->selected_answer)
                                    <div class="inline-flex items-center gap-1 px-3 py-1 bg-rose-50 dark:bg-rose-950/25 text-rose-700 dark:text-rose-450 rounded-full text-xs font-semibold border border-rose-100 dark:border-rose-900/30">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        Salah
                                    </div>
                                    <div class="text-[10px] text-gray-400 dark:text-slate-500 mt-1">Dipilih: <span class="font-bold text-gray-600 dark:text-slate-350">{{ $answer->selected_answer }}</span></div>
                                    <div class="text-[10px] text-emerald-600 dark:text-emerald-400">Kunci: <span class="font-bold">{{ $answer->question->correct_answer }}</span></div>
                                    @else
                                    <div class="inline-flex items-center gap-1 px-3 py-1 bg-slate-50 dark:bg-slate-800 text-gray-500 dark:text-slate-400 rounded-full text-xs font-semibold border border-slate-200 dark:border-slate-700">
                            ...
                            <div class="mt-3 text-xs text-slate-650 dark:text-slate-350 bg-amber-50/50 dark:bg-amber-955/10 border border-amber-100 dark:border-amber-900/20 p-3 rounded-lg flex gap-2">
                                ...
                                <div>
                                    <strong class="text-amber-700 dark:text-amber-500">Penjelasan:</strong> {!! $answer->question->explanation !!}
                                </div>
                            </div>
                            @endif
                        </div>
            ...
            <div class="lg:col-span-1 space-y-6 anim-3">
                ...
                <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        @foreach([
                            'verbal' => ['Verbal', 'bg-blue-50/50 dark:bg-blue-950/20', 'text-blue-600 dark:text-blue-400', 'border-blue-100 dark:border-blue-900/30'],
                            'numerik' => ['Numerik', 'bg-purple-50/50 dark:bg-purple-950/20', 'text-purple-600 dark:text-purple-400', 'border-purple-100 dark:border-purple-900/30'],
                            'logika' => ['Logika', 'bg-orange-50/50 dark:bg-orange-955/20', 'text-orange-600 dark:text-orange-400', 'border-orange-100 dark:border-orange-900/30'],
                            'spasial' => ['Spasial', 'bg-teal-50/50 dark:bg-teal-950/20', 'text-teal-600 dark:text-teal-400', 'border-teal-100 dark:border-teal-900/30']
                ...
                <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-3.5 bg-blue-50/50 dark:bg-blue-955/20 border border-blue-100 dark:border-blue-900/30 rounded-xl text-center">
                            <span class="text-[11px] text-gray-500 dark:text-slate-400 block mb-1">Total Skor</span>
                            <span class="text-xl font-extrabold text-blue-600 dark:text-blue-400">{{ $result->total_score }}%</span>
                        </div>
                        <div class="p-3.5 bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 rounded-xl text-center">
                            <span class="text-[11px] text-gray-500 dark:text-slate-400 block mb-1">Benar</span>
                            <span class="text-xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ $result->total_correct }}</span>
                        </div>
                        <div class="p-3.5 bg-rose-50/50 dark:bg-rose-955/20 border border-rose-100 dark:border-rose-900/30 rounded-xl text-center">
                            <span class="text-[11px] text-gray-500 dark:text-slate-400 block mb-1">Salah</span>
                            <span class="text-xl font-extrabold text-rose-600 dark:text-rose-400">{{ $result->total_wrong }}</span>
                        </div>
                        <div class="p-3.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-150 dark:border-slate-700 rounded-xl text-center">
                            <span class="text-[11px] text-gray-500 dark:text-slate-400 block mb-1">Kosong</span>
                            <span class="text-xl font-extrabold text-slate-600 dark:text-slate-350">{{ $result->total_unanswered }}</span>
                        </div>
                    </div>
                </div>
