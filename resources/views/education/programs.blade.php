        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white">Kelola Program</h2>
                        <p class="mt-2 text-gray-600 dark:text-slate-400">Buat dan kelola program kolaborasi dengan industri untuk
                ...
                <form action="{{ route('education.programs') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                    <div class="relative flex-1">
                        <input type="text" name="search" value="{{ request('search') }}" 
                            placeholder="Cari nama program, jenis..."
            ...
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white">Daftar Program Kerja Sama</h3>
                        ...
                        <div class="p-6 space-y-4">
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div class="flex items-center text-gray-600 dark:text-slate-400 col-span-2">
                                    ...
                                    Mulai: {{ $program['start_date'] ? \Carbon\Carbon::parse($program['start_date'])->format('d M Y') : '-' }}
                                </div>
                            </div>

                            <!-- Industry Partners -->
                            <div>
                                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 mb-2">Mitra Industri:</p>
                            ...
                            <div class="flex gap-3 pt-4 border-t dark:border-slate-800/80">
                                ...
                                <form action="{{ route('education.programs.destroy', $program['id']) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus program ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="p-2 text-red-600 hover:text-red-800 hover:bg-red-50 dark:hover:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-lg transition-colors" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty
                    <div class="col-span-full text-center py-16 bg-white dark:bg-slate-900 rounded-xl shadow border border-gray-100 dark:border-slate-800/80">
                        @if(request('search') || request('status') || request('type'))
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Tidak ada program yang sesuai</h3>
                            <p class="mt-2 text-gray-500 dark:text-slate-400">Coba gunakan kata kunci yang berbeda atau <a href="{{ route('education.programs') }}" class="text-green-600 hover:underline">reset pencarian</a></p>
                        @else
                            ...
                            <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Belum ada program</h3>
                            <p class="mt-2 text-gray-500 dark:text-slate-400">Mulai dengan membuat program kolaborasi pertama Anda.</p>
                            <a href="{{ route('education.programs.create') }}"
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                        <thead class="bg-gray-50 dark:bg-slate-800">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Program</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Durasi</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Peserta</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Tanggal Mulai</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Mitra Industri</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center gap-2">
                                            ...
                                            <form action="{{ route('education.programs.destroy', $program['id']) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus program ini?')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-2 text-red-600 hover:text-red-800 hover:bg-red-50 dark:hover:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-lg transition-colors" title="Hapus">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tr>
                                    <td colspan="8" class="text-center py-12 text-gray-500 dark:text-slate-400">
                                        @if(request('search') || request('status') || request('type'))
                                            ...
                                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Tidak ada program yang sesuai</h3>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Coba gunakan kata kunci yang berbeda atau <a href="{{ route('education.programs') }}" class="text-green-600 hover:underline">reset pencarian</a></p>
                                        @else
                                            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                                            </svg>
                                            <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Belum ada program</h3>
                                            <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Mulai dengan membuat program kolaborasi pertama Anda.</p>
