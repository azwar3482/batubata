<x-guest-layout>
    <div
        ...
        <div class="w-full sm:max-w-md mt-6 px-4 py-6 sm:px-6 sm:py-8 bg-white dark:bg-slate-800 shadow-2xl overflow-hidden sm:rounded-2xl border dark:border-slate-700">
            ...
            <!-- Social Proof -->
            <div class="flex items-center justify-center mb-6 px-3 py-2.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800/30">
                <div class="flex -space-x-2 mr-3">
                    @php $recentUsers = \App\Models\User::whereMonth('created_at', now()->month)->count(); @endphp
                    @for($i = 0; $i < min(3, $recentUsers); $i++)
                        <div class="w-7 h-7 rounded-full bg-{{ ['blue','emerald','indigo'][$i] }}-400 border-2 border-white dark:border-slate-800 flex items-center justify-center text-[10px] font-bold text-white">{{ ['B','S','A'][$i] }}</div>
                    @endfor
                </div>
                <p class="text-xs text-blue-700 dark:text-blue-300 font-medium">
                    <span class="font-bold">{{ number_format($recentUsers) }} orang</span> telah bergabung bulan ini
                </p>
            </div>
