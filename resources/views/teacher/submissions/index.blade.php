<x-app-layout>
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white">Penilaian Tugas</h2>
                <p class="mt-2 text-gray-600 dark:text-slate-400">Lihat dan nilai tugas yang dikumpulkan siswa.</p>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 mb-6 border border-gray-100 dark:border-slate-700">
                <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Kelas</label>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                                ...
                                <td class="px-6 py-4">
                                    @if($submission->status === 'submitted')
                                        <span class="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">Menunggu</span>
                                    @elseif($submission->status === 'graded')
                                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Dinilai</span>
                                    @else
                                        <span class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Revisi</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('teacher.submissions.show', $submission) }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 text-sm font-medium">Detail</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
