    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="{ view: localStorage.getItem('classesView') || 'card' }" x-init="$watch('view', val => localStorage.setItem('classesView', val))">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white">{{ __('messages.manage_classes') }}</h2>
                    <p class="mt-2 text-gray-600 dark:text-slate-400">{{ __('messages.manage_classes_desc') }}</p>
                </div>
            ...
            <div class="mb-6">
                <form action="{{ route('teacher.classes.index') }}" method="GET" class="flex gap-2">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_class_placeholder') }}" class="w-full md:w-1/3 rounded-lg border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                    ...
                    @if(request('search'))
                        <a href="{{ route('teacher.classes.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-700 transition text-sm font-medium">{{ __('messages.reset') }}</a>
                    @endif
                </form>
            </div>
            ...
            <div x-show="view === 'card'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" style="display: none;">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-between text-sm text-gray-500 dark:text-slate-400 mb-4">
                            <span>{{ $class->enrollments_count }}/{{ $class->max_students }} {{ __('messages.students') }}</span>
                            @if($class->start_date)
                        ...
                        <a href="{{ route('teacher.classes.show', $class) }}" class="block w-full text-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition text-sm font-medium">{{ __('messages.manage_class') }}</a>
                    </div>
                </div>
                @empty
                <div class="col-span-full text-center py-12 bg-white dark:bg-slate-800 rounded-xl shadow">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('messages.no_classes_yet') }} {{ request('search') ? __('messages.matching_search') : '' }}</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ __('messages.create_class_desc') }}</p>
                </div>
                @endforelse
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-slate-700 border-b border-gray-100 dark:border-slate-600 text-sm font-medium text-gray-500 dark:text-slate-300">
                                <th class="p-4 w-12">{{ __('messages.no') }}</th>
                                <th class="p-4">{{ __('messages.code') }}</th>
                                <th class="p-4">{{ __('messages.class_name') }}</th>
                                <th class="p-4">{{ __('messages.course') }}</th>
                                <th class="p-4">{{ __('messages.student') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                                ...
                                <td class="p-4 text-right">
                                    <a href="{{ route('teacher.classes.show', $class) }}" class="text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-300 text-sm font-medium">{{ __('messages.manage') }}</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="7" class="p-8 text-center text-gray-500 dark:text-slate-400">{{ __('messages.no_classes_yet') }} {{ request('search') ? __('messages.matching_search') : '' }}</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
