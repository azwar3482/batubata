                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <span class="text-xs text-gray-500 dark:text-slate-400 font-mono">Kode: {{ $class->code }}</span>
                            </div>
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $class->name }}</h1>
                            <p class="mt-1 text-gray-600 dark:text-slate-400">Kursus: <a href="{{ route('teacher.courses.show', $class->course) }}" class="text-blue-600 hover:underline dark:text-blue-400">{{ $class->course->title }}</a></p>
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500 dark:text-slate-400">siswa terdaftar</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700">
                        <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Daftar Siswa</h3>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                                <thead class="bg-gray-50 dark:bg-slate-900/50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Progress</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Status</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Nilai</th>
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50" x-data="{ showStatus: false }">
                                        ...
                                        <td class="px-6 py-4">
                                            @if($enrollment->status === 'active')
                                                <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">Aktif</span>
                                            @elseif($enrollment->status === 'completed')
                                                <span class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">Selesai</span>
                                            @else
                                                <span class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Drop</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $enrollment->final_score ?? '-' }}</td>
                                        <td class="px-6 py-4 text-right">
                                            <button @click="showStatus = !showStatus" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 text-sm font-medium">Kelola</button>
                                            <div x-show="showStatus" x-transition class="absolute right-4 mt-2 p-4 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg z-10 w-72">
                                                <form action="{{ route('teacher.classes.update-student', $enrollment) }}" method="POST" class="space-y-3">
                                                    ...
                                                    <select name="status" class="w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-900 rounded-md">
                                                        ...
                                                        <option value="dropped" {{ $enrollment->status === 'dropped' ? 'selected' : '' }}>Drop</option>
                                                    </select>
                                                    <input type="number" name="final_score" value="{{ $enrollment->final_score }}" placeholder="Nilai (0-100)" min="0" max="100" class="w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-900 rounded-md">
                                                    <textarea name="notes" rows="2" placeholder="Catatan..." class="w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-900 rounded-md">{{ $enrollment->notes }}</textarea>
                                                    <button type="submit" class="w-full px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Simpan</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-gray-500 dark:text-slate-400">Belum ada siswa terdaftar.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Tambah Siswa</h3>
                        <form action="{{ route('teacher.classes.enroll-student', $class) }}" method="POST" class="space-y-3">
                            @csrf
                            <input type="email" name="email" placeholder="Email siswa" required class="w-full text-sm border-gray-300 dark:border-slate-600 dark:bg-slate-900 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <button type="submit" class="w-full px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition text-sm">Daftarkan</button>
                        </form>
                    </div>

                    <!-- Class Info -->
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Info Kelas</h3>
                        <div class="space-y-3 text-sm">
                            @if($class->start_date)
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-slate-400">Mulai</span>
                                <span class="text-gray-900 dark:text-white">{{ $class->start_date->format('d M Y') }}</span>
                            </div>
                            @endif
                            @if($class->end_date)
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-slate-400">Selesai</span>
                                <span class="text-gray-900 dark:text-white">{{ $class->end_date->format('d M Y') }}</span>
                            </div>
                            @endif
                            @if($class->meeting_link)
                            <div>
                                <span class="text-gray-500 dark:text-slate-400">Meeting:</span>
